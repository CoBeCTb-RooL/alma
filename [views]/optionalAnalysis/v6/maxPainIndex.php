...
<style>
    ...
    .t th, .t td{padding: 13px 15px; }
    .t2 th, .t2 td{padding: 6px 10px; text-align: left;  }
...
<script>
    var MaxPain = {
        ...
        list: function(){
            $.ajax({
                url: '/ru/optionalAnalysis/v6/maxPainListAjax',
                data: this.opts,
                beforeSend: function(){ $('.maxPainList').css('opacity', .6); $('.stat-loading').slideDown('fast');  },
                complete: function(){ $('.maxPainList').css('opacity', 1); $('.stat-loading').slideUp('fast');  },
                success: function(data){
                    $('.maxPainList').html(data)
                },
                error: function(){}
            })
        },
        ...
        deleteBunch: function(id){
            if(!confirm('удалить?'))
                return
            $.ajax({
                url: '/ru/optionalAnalysis/v6/maxPainDeleteBunchAjax',
                data: {id: id},
...
<!--ФОРМА-->
<form class="form" action="/ru/optionalAnalysis/v6/maxPainFormSubmit" id="addZoneForm" target="frame7" onsubmit="if(confirm('Сохранить данные?')){return true; } return false; " style="position: fixed; top: 0px; right: 0px; background: #efefef; ">
...
<div class="maxPainList">Загрузка...</div>
