...
class OptionalAnalysisItem{
	...
	function __construct($arr)
	{
		if(count($arr))
		{
			...
			$this->strikeType = StrikeType::code($arr['strikeType']);
			$this->strike = $arr['strike'];
			$this->premium = $arr['premium'];
			$this->forward = $arr['forward'];

			/*$this->currencyStr = $arr['currency'];
			$this->typeStr = $arr['type'];
			$this->strikeTypeStr = $arr['strikeType'];*/
	...
	{
		self::deleteByDateAndCurrencyAndStrijeTypeAndType($this->dt, $this->currency->code, $this->strikeType->code,  $this->type->code);
	}



	public static function deleteByDateAndCurrencyAndStrijeTypeAndType($date, $cur, $strikeType, $type)
	{
